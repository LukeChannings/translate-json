...
const dedupeMarkers = (markers) => {
  ...
  return {dupeCount, dedupedMarkers}
}

async function translateDeep ({doc, transforms = [], options: {verbose, concurrency}}) {
  const trMarkers = []
  ...
  const translations = await map(
    dedupedMarkers.values(),
    ({tr}) => tr(),
    {concurrency}
  )
