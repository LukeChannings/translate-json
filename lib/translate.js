...
const dedupeMarkers = (markers) => {
  ...
  for (const {value, path, tr} of markers) {
    ...
    if (existing) {
      ...
      dedupedMarkers.set(value, {tr, paths: [path]})
    }
  }

  return {duplicationsCount, dedupedMarkers}
}

async function translateDeep ({doc, transforms = [], verbose}) {
  const trMarkers = []
  ...
  const {dedupedMarkers, duplicationsCount} = dedupeMarkers(trMarkers)

  if (verbose) console.info(format(messages.DUPLICATE_COUNT, duplicationsCount))

  const translations = await map(dedupedMarkers.values(), ({tr}) => tr(), {concurrency: 40})
